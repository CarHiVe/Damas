...
Tablero JugadorHumano::movimientoBlanca(const Tablero& t){
    ...
    Tablero aux = Tablero(t);
        do{
            list<Tablero> comeReina;
            list<Tablero> come;
            auxiliar.operator =(aux.getMovimiento(jugador, true));
            list<Tablero>::iterator ini = auxiliar.begin();
            list<Tablero>::iterator fin = auxiliar.end();
            //Deseo saber cual es el numero maximo de fichas comidas
            int cuentas = 0;
            while(ini != fin){
            ...
            ini = auxiliar.begin();
            fin = auxiliar.end();
            //guardar los tableros sólo si hay reinas y coman mas de dos fichas
            while(ini != fin){
                if(cuentas >= 2 && ini->reinaBlanca == true)
                    comeReina.push_back(*ini);
                ini++;
            }
            
            list<Tablero>::reverse_iterator inir = auxiliar.rbegin();
            list<Tablero>::reverse_iterator finr = auxiliar.rend();
            //Si en el bucle anterior no se halló ninguno, 
            if(comeReina.empty()){
                int cantTab = 0;
                while(inir != finr){
                    
                    if(inir->cuantasCome == cuentas && cantTab == 0)
                        cantTab = cuentas;
            ...
            //wcout << "Cuantas come " << comeReina.size() << endl;
            move[0] = 'n';
            move[1] = 'n';
            move[2] = 'n';
            move[3] = 'n';
            ...
            if(!come.empty() || !comeReina.empty()){
                wcout << "\nUsted es " << es << ". Ingrese coordenadas (por ejemplo: h4g3) para comer: ";
                wcin >> setw(1) >> move[0];
                wcin >> setw(1) >> move[1];
                wcin >> setw(1) >> move[2];
                wcin >> setw(1) >> move[3];
                ...
                if(((int)move[0] > 64 && (int)move[0] < 75) && ((int)move[2] > 64 && (int)move[2] < 75)){
                    if((Y1 >= 0 && Y1 < 10) && (Y2 >= 0 && Y2 < 10)){
                        ...
                        if(pieza == 1){
                            ...
                            if(temporal.salto(X1, Y1, X1-1, Y1-1) && X1-2 == X2 && Y1-2 == Y2){
                                temporal = Tablero(temporal, X1, Y1, 4);
                                ...
                                while(iniCome != finCome){
                                    if(iniCome->equals(temporal) == true)
                                        existe = true;
                                    iniCome++;
                                }
                                
                                if(existe == true){
                                    aux.operator =(temporal);
                                    list<Tablero> comer;
                                    auxiliar.operator =(aux.getMovimiento(jugador, true));
                                    list<Tablero>::iterator ini = auxiliar.begin();
                                    list<Tablero>::iterator fin = auxiliar.end();
                                ...
                                if(temporal.salto(X1, Y1, X1-1, Y1+1) && X1-2 == X2 && Y1+2 == Y2 ){
                                    temporal = Tablero(temporal, X1, Y1, 5);
